<template>
  ...
  >
    ...
    <div class="flex flex-col lg:w-2/4 w-[100%] mb-2 justify-center pl">
      <div class="mb-6">
        <label class="block text-sm font-bold mb-2" for="task"> Task </label>
        <input
          :class="[
            'shadow',
            taskError ? 'border-red-700' : '',
            'text-gray-700',
            'appearance-none',
            'border-2',
            'rounded',
          ...
          type="text"
          v-model="newTask"
          @change="inputChange('Task')"
          placeholder="Add a task"
        />
        <p v-if="taskError" class="text-red-700 text-xs italic">
          Please add a task.
        </p>
      </div>
      <div class="mb-6">
        ...
        <input
          ...
          type="date"
          v-model="dueDate"
          @change="inputChange('Date')"
          placeholder="DD-MM-YYYY"
        />
        <p v-if="dueDateError" class="text-red-700 text-xs italic">
          Please choose a due date.
        </p>
      </div>
    </div>
    <div
      class="text-md mt-8 lg:w-2/4 w-[100%] py-2 px-6 flex flex-row justify-evenly items-center"
    >
      <button id="closeModal" class="text-white" @click="toggleModal">
        Close
      </button>

      <button class="text-orange-400" @click="addTask(mode)">
        {{ this.mode === "Create" ? "Add" : "Update" }}
      </button>
    </div>
  </div>
</template>
