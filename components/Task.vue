<template>
  <div
    :class="[
      ...
      'rounded-md',
      'border-solid',
      pastDueDate ? 'border-red-700' : 'border-sky-950',
      'border-2',
      'mb-3',
    ]"
  >
    <div class="flex lg:flex-row flex-col items-center justify-around h-3/6">
      <div id="content" class="font-bold text-xl">
        {{ task.content }}
      </div>
      <div id="dueDate" class="font-bold text-xl">
        {{ task.dueDate }}
      </div>
    </div>
    <div class="h-3/6 flex flex-row items-center justify-evenly">
      <div
        class="flex lg:flex-row lg:justify-around flex-col justify-center items-center w-1/2"
      >
        <button
          id="delete"
          @click="toggleDelete"
      ...
      <div
        class="flex lg:flex-row flex-col lg:justify-around justify-center items-center w-1/2"
      >
        <button
          id="duplicate"
          v-if="!task.done"
    ...
    <div>
      <p
        id="pastduedate"
        v-if="pastDueDate"
        class="ml-5 mb-5 text-red-700 text-xs italic"
      >
        Past due date.
      </p>
    </div>
